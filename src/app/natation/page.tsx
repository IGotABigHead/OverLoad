...
type CreateNatationActivity = Omit<NatationActivity, '_id'>;
...
export default function NatationPage() {
  ...
  const handleDeleteActivity = async (id: string) => {
    try {
      const response = await fetch(`/api/natation/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:ml-20 mt-30">
      <h1 className="text-3xl font-bold mb-8">Mes séances de natation</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <h2 className="text-xl font-semibold mb-4">Ajouter une séance</h2>
          <NatationActivityForm onSubmit={handleAddActivity} />
        </div>

        <div>
          <h2 className="text-xl font-semibold mb-4">Mes séances</h2>
          <NatationActivityList activities={paginatedActivities} onDelete={handleDeleteActivity} />
          {totalPages > 1 && (
            <Pagination
